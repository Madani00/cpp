...
std::vector<int> jacob_numbers(size_t sizePend) {
    ...
    {
        nextNm = jacob_numbers[i] + 2 * jacob_numbers[i-1]; // 11
        if (nextNm >= sizePend)
            break;
...
std::vector<int> generate_jacob_order(size_t sizePend) {
    ...
    std::cout << "- Jacob numbers numbers: ";
    PrintVecInline(jacob_numb);
    std::cout << std::endl;
...
int main(int ac, char *av[]) 
{
    ...
    if (ac > 1) {
        ...
        std::vector<int> sortedVec = merge_insertion(Myvector); // to test
